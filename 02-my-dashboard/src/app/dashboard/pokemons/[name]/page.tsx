...
export async function generateStaticParams() {
    try {
        const pokemons: PokemonsResponse = await fetch('https://pokeapi.co/api/v2/pokemon?limit=151&offset=0').then(res => res.json());

        return pokemons.results.map(poke => ({
            name: poke.name
        }));
    } catch(error) {
        console.error('Error on generate static params');
    }
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    try {
        ...
        return {
            title: 'Error - Pokémon',
            description: 'Error al consultar el Pokémon'
        }
    }
}
